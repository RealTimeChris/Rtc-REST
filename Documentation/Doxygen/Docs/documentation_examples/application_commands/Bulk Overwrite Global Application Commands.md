...
============
- Execute the `rtc_rest::application_commands::bulkOverwriteGlobalApplicationCommandsAsync()` function, while passing in a data structure of type `rtc_rest::bulk_overwrite_global_application_commands_data`, with a return value of `auto` or `std::vector<rtc_rest::application_command_data>`.
