...
#pragma once
	...
	bool areWeInADM(input_event_data eventData, channel channel, bool displayResponse = true) {
		...
		if (currentChannelType == channel_type::dm) {
			if (displayResponse) {
				...
				std::unique_ptr<embed_data> msgEmbed(std::make_unique<embed_data>());
				...
				std::unique_ptr<respond_to_input_event_data> responseData(std::make_unique<respond_to_input_event_data>(eventData));
	...
	bool checkIfAllowedPlayingInChannel(input_event_data eventData, discord_guild discordGuild) {
		...
		if (discordGuild.data.musicChannelIds.size() > 0) {
			...
			if (isItFound == false) {
				...
				std::unique_ptr<respond_to_input_event_data> replyMessageData(std::make_unique<respond_to_input_event_data>(eventData));
	...
	bool checkIfWeHaveControl(input_event_data eventData, discord_guild guildData, guild_member_data guildMember) {
		...
		if (!doWeHaveControl) {
			...
			std::unique_ptr<respond_to_input_event_data> dataPackage(std::make_unique<respond_to_input_event_data>(eventData));
