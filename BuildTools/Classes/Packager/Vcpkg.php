...
class Vcpkg
{
    ...
    /**
     * Check out a repository by tag or branch name to ~/rtc_rest,
	...
	$repositoryUrl = 'https://' . urlencode($argv[1]) . ':' . urlencode($argv[2]) . '@github.com/RealTimeChris/Rtc-REST';

        echo GREEN . "Check out repository: $tag (user: ". $argv[1] . " branch: " . $tag . ")\n" . WHITE;

        chdir(getenv('HOME'));
        system('rm -rf ./rtc_rest');
        ...
        $this->git('clone ' . escapeshellarg($repositoryUrl) . ' ./rtc_rest --depth=1');
        
        /* This is noisy, silence it */
	$status = chdir(getenv("HOME") . '/rtc_rest');
    ...
    /**
     * Create ./Vcpkg/ports/rtc_rest/vcpkg.json and return the portfile contents to
     * build the branch that is cloned at ~/rtc_rest
     * 
  ...
  "dependencies": [
    ...
    },
    {
      ...
      "host": true
        ...
        file_put_contents('./Vcpkg/ports/rtc_rest/vcpkg.json', $versionFileContent);
    ...
    function firstBuild(string $portFileContent): string
    {
        ...
        chdir(getenv("HOME") . '/rtc_rest');
        echo GREEN . "Create /usr/local/share/vcpkg/ports/rtc_rest/\n" . WHITE;
        $this->sudo('mkdir -p /usr/local/share/vcpkg/ports/rtc_rest/');
        echo GREEN . "Copy vcpkg.json to /usr/local/share/vcpkg/ports/rtc_rest/vcpkg.json\n" . WHITE;
        $this->sudo('cp -v -R ./Vcpkg/ports/rtc_rest/vcpkg.json /usr/local/share/vcpkg/ports/rtc_rest/vcpkg.json');
        file_put_contents('/tmp/portfile', $portFileContent);
        $this->sudo('cp -v -R /tmp/portfile /usr/local/share/vcpkg/ports/rtc_rest/portfile.cmake');
        unlink('/tmp/portfile');
        $buildResults = shell_exec($this->sudo . ' /usr/local/share/vcpkg/vcpkg install rtc_rest:x64-linux');
        $matches = [];
    ...
    function secondBuild(string $portFileContent): bool
    {
        ...
        chdir(getenv("HOME") . '/rtc_rest');
        file_put_contents('./Vcpkg/ports/rtc_rest/portfile.cmake', $portFileContent);
        $this->sudo('cp -v -R ./Vcpkg/ports/rtc_rest/vcpkg.json /usr/local/share/vcpkg/ports/rtc_rest/vcpkg.json');
        $this->sudo('cp -v -R ./Vcpkg/ports/rtc_rest/portfile.cmake /usr/local/share/vcpkg/ports/rtc_rest/portfile.cmake');
        $this->sudo('cp -v -R ./Vcpkg/ports/* /usr/local/share/vcpkg/ports/');

        echo GREEN . "vcpkg x-add-version...\n" . WHITE;
        chdir('/usr/local/share/vcpkg');
        $this->sudo('./Vcpkg format-manifest ./ports/rtc_rest/vcpkg.json');
        /* Note: We commit this in /usr/local, but we never push it (we can't) */
        $this->git('add .', true);
        $this->git('commit -m "[bot] VCPKG info update"', true);
        $this->sudo('/usr/local/share/vcpkg/vcpkg x-add-version rtc_rest');

        echo GREEN . "Copy back port files from /usr/local/share...\n" . WHITE;
        chdir(getenv('HOME') . '/rtc_rest');
        system('cp -v -R /usr/local/share/vcpkg/ports/rtc_rest/vcpkg.json ./Vcpkg/ports/rtc_rest/vcpkg.json');
        system('cp -v -R /usr/local/share/vcpkg/versions/d-/rtc_rest.json ./Vcpkg/versions/d-/rtc_rest.json');
        ...
        $resultCode = 0;
        $output = [];
        exec($this->sudo . ' /usr/local/share/vcpkg/vcpkg install rtc_rest:x64-linux', $output, $resultCode);

        if ($resultCode != 0) {
            echo RED . "There were build errors!\n\nBuild log:\n" . WHITE;
            readfile("/usr/local/share/vcpkg/buildtrees/rtc_rest/install-x64-linux-dbg-out.log");
        }
